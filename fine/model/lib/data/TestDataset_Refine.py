...
class TestDataset_Refine(Dataset):
    ...
    def __init__(self, opt, phase='train'):
        ...
        self.DEPTH_FRONT = os.path.join(self.root, 'DEPTH_FRONT')
        self.DEPTH_BACK_PLUS = os.path.join(self.root, 'DEPTH_BACK_PLUS')
    ...
    def get_img_info(self, subject):
        ...
        fdepth_path = os.path.join(self.DEPTH_FRONT, subject+'.png')
        bdepth_path = os.path.join(self.DEPTH_BACK_PLUS, subject+'.png')

        render = Image.open(render_path).convert('RGB')
        fnorm = Image.open(fnorm_path).convert('RGB')
        fdepth = Image.open(fdepth_path).convert('RGB')
        bdepth = Image.open(bdepth_path).convert('RGB')
        render = render.resize((256, 256))
        fnorm = fnorm.resize((256, 256))
        fdepth = fdepth.resize((256, 256))
        bdepth = bdepth.resize((256, 256))
        ...
        bdepth = self.to_tensor(bdepth)

        render = torch.cat([render, fnorm, fdepth, bdepth], 0)
