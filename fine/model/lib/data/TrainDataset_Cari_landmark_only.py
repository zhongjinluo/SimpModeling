...
class TrainDataset_Cari_Landmark_Only(Dataset):
    ...
    def __init__(self, opt, phase='train'):
        ...
        self.B_MIN = np.array([-10, -10, -10])
        self.B_MAX = np.array([10, 10, 10])
    ...
    def select_sampling_method(self, subject):
        ...
        surface_points = landmarks_GT[0]

        sample_points = surface_points + np.random.normal(scale=self.opt.sigma, size=[surface_points.shape[0]*self.num_sample_inout // 4, 3])
        #print('=== sample_points: ', sample_points.shape)
        #sample_labels = np.ones((1, sample_points.shape[0]))
        # add random points within image space
        length = self.B_MAX - self.B_MIN
        #random_points = np.random.rand(self.num_sample_inout // 4, 3) * length + self.B_MIN
        random_points = (np.random.rand(self.num_sample_inout // 4, 3)-0.5) * length + surface_points
        sample_points = np.concatenate([sample_points, random_points], 0)
                
        points_label = []
        for i in range(sample_points.shape[0]):
            for j in range(1):
                if np.linalg.norm(sample_points[i, :] - landmarks_GT[j, :], ord=2)<5:
                    points_label.append(i)
                    break
                            
        #print('=== points_label: ', len(points_label))

        inside_points = sample_points[points_label]
        #print('=== inside_points: ', inside_points.shape)
        temp = np.ones(sample_points.shape[0], dtype=bool)
        temp[points_label]=False
        #print('=== logical: ', temp)
        outside_points = sample_points[temp]
        #print('=== outside_points: ', outside_points.shape)
        
        x = np.arange(0, sample_points.shape[0], 1)
        np.random.shuffle(x)
        #print('=== x: \n', x)
        ...
        sample_labels = np.concatenate([np.ones((1, inside_points.shape[0])), np.zeros((1, outside_points.shape[0]))], 1)
