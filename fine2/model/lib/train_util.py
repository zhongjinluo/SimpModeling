...
def gen_mesh_with_template(opt, net, cuda, data, save_path, use_octree=True, vertices=None, faces=None, constraint=None, constraint_faces=None, part_boundary=None, inner_constraints=None, is_global=True):
    ...
    Image.fromarray(np.uint8(save_img[:,:,9:12])).save(save_path[:-4] + '_s.png')
    Image.fromarray(np.uint8(save_img[:,:,6:9])).save(save_path[:-4] + '_n.png')
    Image.fromarray(np.uint8(save_img[:,:,3:6])).save(save_path[:-4] + '_d.png')
    Image.fromarray(np.uint8(save_img[:,:,:3])).save(save_path[:-4] + '_db.png')
    end = time.time()
    ...
    if is_global:
        print("global")
        verts, faces = reconstruction_with_template_global(
            net, cuda, calib_tensor, opt.resolution, b_min, b_max, use_octree=use_octree, vertices=vertices, faces=faces, constraint=constraint, constraint_faces=constraint_faces, part_boundary=part_boundary, inner_constraints=inner_constraints)
    else:
        print("local")
    ...
    # mesh.simplify_quadratic_decimation(mesh.faces.shape[0]*0.9)
    # if is_global:
    #     trimesh.smoothing.filter_laplacian(mesh, iterations=1)
    mesh.export(save_path.replace("pifu.obj", "pifu_move.obj"))
    end = time.time()
    print("smoothing:", end-start)
...
def gen_mesh(opt, net, cuda, data, save_path, use_octree=True):
    ...
    try:
        ...
        save_img = np.concatenate(save_img_list, axis=1)
        #Image.fromarray(np.uint8(save_img[:,:,::-1])).save(save_img_path)
        Image.fromarray(np.uint8(save_img[:,:,6:9])).save(save_img_path)
        Image.fromarray(np.uint8(save_img[:,:,3:6])).save(save_path[:-4] + '_f.png')
        Image.fromarray(np.uint8(save_img[:,:,:3])).save(save_path[:-4] + '_b.png')
